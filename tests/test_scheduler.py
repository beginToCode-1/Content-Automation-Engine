...
from content_engine.db import connection, runs_repo, schedules_repo, uploads_repo
...
def _fake_submit_run(run_id_to_return):
    def _submit(settings, **kwargs):
        runs_repo.insert_run(
            connection.get_pool(), run_id_to_return, kwargs["topic"], kwargs["trigger_source"], kwargs["target_platforms"]
        )
...
def test_poll_once_always_forces_private_regardless_of_entry(pg_pool, tmp_path):
    settings = _fake_settings(tmp_path)
    schedules_repo.insert_schedule(
        pg_pool, "topic-x", "once", ["youtube"], scheduled_time="2000-01-01T00:00:00.000000Z"
    )
...
def test_poll_once_marks_once_schedule_completed(pg_pool, tmp_path):
    settings = _fake_settings(tmp_path)
    schedule_id = schedules_repo.insert_schedule(
        pg_pool, "topic-y", "once", ["youtube"], scheduled_time="2000-01-01T00:00:00.000000Z"
    )
    ...
    active = schedules_repo.list_active(pg_pool)
    entry = next(e for e in active if e["id"] == schedule_id)
    assert entry["status"] == "completed"
    assert entry["last_run_id"] == "run456"


def test_poll_once_no_due_entries_does_not_submit(pg_pool, tmp_path):
    settings = _fake_settings(tmp_path)
    schedules_repo.insert_schedule(
        pg_pool, "topic-z", "once", ["youtube"], scheduled_time="2999-01-01T00:00:00.000000Z"
    )
...
def test_poll_once_processes_due_queued_uploads(pg_pool, tmp_path):
    settings = _fake_settings(tmp_path)
    runs_repo.insert_run(pg_pool, "run1", "topic", "web", ["youtube"])
...
def test_poll_once_ignores_future_scheduled_uploads(pg_pool, tmp_path):
    settings = _fake_settings(tmp_path)
    runs_repo.insert_run(pg_pool, "run1", "topic", "web", ["youtube"])
    uploads_repo.insert_pending_uploads(pg_pool, "run1", ["youtube"])
